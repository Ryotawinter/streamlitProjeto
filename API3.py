...
def gather_all_data(puuid, match_id, mass_region, api_key, role, tempo1, tempo2,summoner_name):
    try:
        data = {'Nick': [],
                'Data': [],
                'champion': [],
        ...
        if int(minutos) > 15:
            ...
            if position == role:
                ...
                print(f'Chegou aqui{danoporgold}')
    ...
    with concurrent.futures.ThreadPoolExecutor(max_threads) as executor:
        ...
        for match_id in match_ids:
            ...
            if api_key==api_keys[1]:
                ...
                puuid2 = puuid
            future = executor.submit(gather_all_data,puuid2, match_id, mass_region, api_key, role, tempo1, tempo2,summoner_name)
...
BOTTOM = ["HKS shima og", "AWE Fujita", "eihei", "Rabelokinho", "Tessin1", "Tessin o matador"]
...
def dataframe(lista,api_keys,start_time):
    try:
        data = pd.DataFrame()
        number=0
        for i in lista:
    ...
    try:
        ...
        for i in lista:
            ...
            for j in i:
                ...
                print(summoner_name)
                region='BR1'
                df=rank(summoner_name,region,api_keys[0])
                pdl = pd.concat([pdl, df], ignore_index=True)
                time.sleep(1)
                print(pdl)
            number+=1
        fim=time.time()
...
data=dataframe(lista,api_keys,start_time)
data.to_pickle(nome_arquivo)
#pdlgeral=pdl(lista,api_keys)
#pdlgeral.to_csv(nome_arquivo1, index=False)
